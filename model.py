...
class SkipGramModel(nn.Module):
    ...
    def __init__(self, emb_size, emb_dimension, embedding_type):
        ...
        self.u_embeddings = get_embedding(
            emb_size, emb_dimension)  # , sparse=True
        self.v_embeddings = nn.Embedding(
            emb_size, emb_dimension)  # , sparse=True

        initrange = 1.0 / self.emb_dimension
        if type(self.u_embeddings) == nn.Embedding:
            init.uniform_(self.u_embeddings.weight.data,    -
                          initrange, initrange)
        else:
    ...
    def load_embeddings(self, id2word, path):
        ...
        self.u_embeddings.weight = torch.nn.Parameter(
